    ...
    >
      <div className=" bottom-0 left-0 w-full backdrop-blur-xl bg-white/10 border-t border-white/20 text-black text-center py-4 z-50">
        <div className="max-w-screen-2xl mx-auto px-4 md:px-20">
            ...
            {socialLinks.map((item, index) => (
              ...
              >
                <a href={item.link} target="_blank" rel="noopener noreferrer">
                  <item.icon
                    size={25}
                    className={`text-black hover:scale-150 duration-300 ${item.color}`}
                  />
                </a>
              </motion.li>
            ))}
          ...
          {/* Contact Info */}
          <motion.div
            className="text-black flex flex-col md:flex-row items-center justify-center gap-4 cursor-pointer hover:text-green-400 mt-6"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: false, amount: 0.2 }}
            variants={fadeInUp}
          ...
          <hr className="text-blue-500 shadow-2xl my-4" />
